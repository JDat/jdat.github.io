...
function getSitePredictions(stationID) {

  let stationIndex  = -1;
  for (const i in siteList) {
    if (siteList[i].station == stationID) stationIndex = i;
  }

  if (stationIndex === -1) {
    console.log('Error searching stationID');
    return;
  }

  console.log(siteList[stationIndex]);
  for (const i in siteList[stationIndex].times) {
    let launchTimeArray=siteList[stationIndex].times[i].split(':');
    console.log(launchTimeArray);
    const d = new Date;

    d.setDate(d.getUTCDate()+1);
    d.setUTCHours(launchTimeArray[1]);
    d.setUTCMinutes(launchTimeArray[2]);
    d.setUTCSeconds(0);
    d.setUTCMilliseconds(0);
    // https://api.v2.sondehub.org/tawhiri?launch_latitude=56.6425&launch_longitude=25.128300000000024
    // &launch_datetime=2022-05-09T11:15:00Z&ascent_rate=6.7&burst_altitude=25300&descent_rate=12.9
    let fetchUrl = 'https://api.v2.sondehub.org/tawhiri?launch_latitude=';
    fetchUrl += siteList[stationIndex].position[1];
    fetchUrl += '&launch_longitude=';
    fetchUrl += siteList[stationIndex].position[0];
    fetchUrl += '&launch_datetime=';
    fetchUrl += d.toISOString();
    fetchUrl += '&ascent_rate=';
    fetchUrl += siteList[stationIndex].ascent_rate ? siteList[stationIndex].ascent_rate : 5;
    fetchUrl += '&burst_altitude=';
    fetchUrl += siteList[stationIndex].burst_altitude ? siteList[stationIndex].burst_altitude : 24000;
    fetchUrl += '&&descent_rate=';
    fetchUrl += siteList[stationIndex].descent_rate ? siteList[stationIndex].descent_rate : 12.5;

    console.log(fetchUrl);
    fetchSitePrediction(fetchUrl);
  }
}

function fetchSitePrediction(fetchUrl) {
  fetch(fetchUrl).then(function (response) {
    response.json().then(function (result) {
      //console.log(result);
      if (result.prediction.length == 2) {
        let polyline = [];
        for (let index = 0; index < result.prediction[0].trajectory.length; index++) {
          const entry = result.prediction[0].trajectory[index];
          polyline.push({lat: entry['latitude'], lon: entry['longitude'], alt: entry['altitude']});
        }
        for (let index = 0; index < result.prediction[1].trajectory.length; index++) {
          const entry = result.prediction[1].trajectory[index];
          polyline.push({lat: entry['latitude'], lon: entry['longitude'], alt: entry['altitude']});
        }

        let path = ui.addPath(polyline, 'sitePredict');
      }
    });
  });
}
    ...
    response.json().then(function (result) {
      ...
      for (const key in result) {
        ...
        if (inRange(loc)) {
          ...
          const timeoutMillis = 1000 + Math.floor(Math.random() * 2500);
          setTimeout(function() {
      ...
      for (const key in sondeList) {
        ...
        const timeoutMillis = 1500 + Math.floor(Math.random() * 4500);
        setTimeout(function() {
